...
describe('GetQuizQuestionsUseCase', () => {
  ...
  describe('when the quiz exists', () => {
    it('returns quiz with its questions', async () => {
      quizRepo.seed(new Quiz('q1', 'doc1', 'Test Quiz', 2, FIXED_DATE, FIXED_DATE));
      questionRepo.seed(
        new Question('qn1', 'q1', QuestionType.MULTIPLE_CHOICE, 'Q1?', [
          { label: 'A', text: 'Yes', isCorrect: true },
          { label: 'B', text: 'No', isCorrect: false },
        ], 1, 'easy', FIXED_DATE),
        new Question('qn2', 'q1', QuestionType.TRUE_FALSE, 'Q2?', [
