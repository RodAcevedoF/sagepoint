...
const verifyToken = async (token: string) => {
  const url = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001'}/auth/verify?token=${encodeURIComponent(token)}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(payload?.message ?? res.statusText ?? 'Verification failed');
  }
