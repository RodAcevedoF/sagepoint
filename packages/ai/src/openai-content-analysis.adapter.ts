...
export class OpenAiContentAnalysisAdapter implements IContentAnalysisService {
  ...
  async extractConcepts(text: string): Promise<ExtractedConcept[]> {
    ...
    try {
      this.logger.log(
        `Extracting concepts from text (${text.length} chars) using LangChain`,
      );
      ...
      const conceptSchema = z.object({
        concepts: z.array(
          z.object({
            name: z
              .string()
              .describe("The name of the concept (concise, singular)"),
            description: z
              .string()
              .describe("A brief description of the concept"),
            relationships: z
              .array(
                z.object({
                  targetName: z
                    .string()
                    .describe(
                      "The name of the related concept (must match another concept's name EXACTLY)",
                    ),
                  type: z
